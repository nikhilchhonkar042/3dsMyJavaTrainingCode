...
/*
	// returning object // taking object as parameter
	BankAccount applyForABankAccount(BankApplicationForm bankAppFormRef) {
		System.out.println("reciving detalis :"+ bankAppFormRef.getApplicantName() );
		
		System.out.println("Creating a new account ");
		
		BankAccount newUserBankAccObj1 = new BankAccount();
		
		
		double value = Math.random()*1000;
		long accNum =  (long) (value);
		newUserBankAccObj1.setAccountNumber(accNum);
		newUserBankAccObj1.setAccountBalance(2500);
		newUserBankAccObj1.setAccountHolder(bankAppFormRef.getApplicantName());
		newUserBankAccObj1.setAccountType(bankAppFormRef.getAccountType());
		newUserBankAccObj1.setIfscCode("HDFC12344");
		newUserBankAccObj1.setOperationType("single");
		return newUserBankAccObj1;
		
		}
	
	
	BankAccount  applyForJointAccount(BankApplicationForm User1, BankAccount User2) {
		System.out.println("reciving detalis from both the user:"  );
		BankAccount newJointBankAccObj1 = new BankAccount();
		double value = Math.random() * 1000;
		long accNum = (long) value;
		newJointBankAccObj1.setAccountNumber(accNum);//real time, connect to the DB, and find out the last account number generated, and one 
		newJointBankAccObj1.setAccountHolder(User1.getApplicantName()+" "+User2.getApplicantName());
		newJointBankAccObj1.setAccountType(User1.getAccountType());
		newJointBankAccObj1.setIfscCode("BOB000000015");
		newJointBankAccObj1.setAccountBalance(5000);
		newJointBankAccObj1.setOperationType("Joint");
		return newJointBankAccObj1;
	}
	*/
