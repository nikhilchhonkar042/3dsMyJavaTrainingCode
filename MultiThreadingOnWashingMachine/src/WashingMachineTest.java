...
//error 4
class Cyclone extends RuntimeException {
    Cyclone(String msg) {
        super(msg);
    }
}
...
//error 5
class Flood extends RuntimeException {
    Flood (String msg) {
        super(msg);
    }
}
...
class WashingMachine implements Runnable {
    ...
    public void run() {
        ...
        if (voltageDrop) {
            throw new VoltageDrop("Voltage Drop is detected in Washing Machine " + machineNumber);
            
        }
        double value2 = Math.random() % 10;
        if ( value2 > 0.25 && value2 < 0.35) {
        	cyclone = true;
        } else {
            System.out.println("No Cyclone Washing Machine"+ machineNumber+"Just fine");
            
        }

        if (cyclone) {
            throw new Cyclone("Cyclone is detected in Washing Machine to gya " + machineNumber);
        ...
        if (value3 > 0.85) {
            ...
            System.out.println("No flood Washing Machine"+ machineNumber+"Just fine");
            
        }

        if (flood) {
            throw new Flood("flood is detected cloths are remain unwash  in Washing Machine to gya " + machineNumber);
